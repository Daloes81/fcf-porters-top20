...
export async function getClassification(url) {
  ...
  $('table').each((_, t) => {
    ...
    if (allText.includes('Partits') && allText.includes('Punts') && $(t).find('tr').length > 3) {
      table = t;
    }
  });
  ...
  $(table)
    ...
    .each((i, tr) => {
      ...
      const J  = parseInt($(tds[6]).text().trim(),  10) || 0;
      const GF = parseInt($(tds[19]).text().trim(), 10) || 0;
      const GC = parseInt($(tds[20]).text().trim(), 10) || 0;
...
// ---------------------------------------------------------------------------
// 3) ACTA -> identifica el porter per color de samarreta diferent i si
//    l'equip va encaixar gols (per calcular el % de partits a 0)
// ---------------------------------------------------------------------------
export async function parseActaForGoalkeeper(actaUrl, teamName) {
  ...
  // Recollim TOTS els jugadors convocats de l'equip (titulars + suplents)
  // per detectar el porter pel color de samarreta diferent (minoritari).
  // L'estructura de l'acta és: Titulars[local], Suplents[local], ...,
  // Titulars[visitant], Suplents[visitant] — cada etiqueta apareix 2 vegades.
  const teamIndex = isHome ? 0 : 1;
  const titularTables = [];
  const suplentTables = [];
  $('table').each((_, t) => {
    const headTxt = $(t).find('tr').first().text().trim();
    if (headTxt === 'Titulars') titularTables.push(t);
    else if (headTxt === 'Suplents') suplentTables.push(t);
  });

  const tablesToScan = [
    titularTables[teamIndex],
    suplentTables[teamIndex]
  ].filter(Boolean);
  if (tablesToScan.length === 0) return { gk: null, conceded };
  ...
  for (const tbl of tablesToScan) {
    $(tbl)
      ...
      .each((i, tr) => {
        ...
        if (name && color) rows.push({ name, url: abs(a.attr('href')), color });
      });
  }

  // Identificació del porter exclusivament per color de samarreta:
  // el porter (i l'eventual porter suplent) vesteixen un color diferent
  // (minoritari) de la resta de jugadors de camp convocats.
  let gkRow = null;
  const colorCounts = new Map();
  rows.forEach((r) => colorCounts.set(r.color, (colorCounts.get(r.color) || 0) + 1));

  if (colorCounts.size > 1) {
    const sorted = [...colorCounts.entries()].sort((a, b) => a[1] - b[1]);
    const [minorColor, minorCount] = sorted[0];
    const [, majorCount] = sorted[sorted.length - 1];
    if (minorCount < majorCount) {
      gkRow = rows.find((r) => r.color === minorColor) || null;
    }
  }

  const gk = gkRow ? { name: gkRow.name, url: gkRow.url } : null;
