...
export async function parseActaForGoalkeeper(actaUrl, teamName) {
  ...
  // Recollim els jugadors titulars de l'equip per detectar el porter
  // pel color de samarreta diferent (minoritari) de la resta.
  const titularTables = [];
  $('table').each((_, t) => {
  ...
  $(table)
    ...
    .each((i, tr) => {
      ...
      if (name) rows.push({ name, url: abs(a.attr('href')), color });
    });

  // Identificació del porter exclusivament per color de samarreta:
  // el porter vesteix un color diferent (minoritari) de la resta
  // de jugadors de camp titulars.
  let gkRow = null;
  const colorCounts = new Map();
  rows.forEach((r) => {
    if (r.color) colorCounts.set(r.color, (colorCounts.get(r.color) || 0) + 1);
  });
